...
- 내가 직접 터미널 명령을 실행하라고 시키지 마. 필요한 명령은 네가 실행해. 단 하나의 예외는 5-2단계의 Claude Code 로그인이야. 그 로그인은 터미널 화면 안에서 진행되고 본인이 직접 완료해야 하므로, 그때만 내가 할 일을 안내해 줘.
- 설치에는 공식 출처만 사용해. Claude Code는 `https://claude.ai/install.ps1` 공식 설치 스크립트만, GitHub CLI는 winget의 `GitHub.cli` 공식 패키지만 사용해. 이 두 경로 외의 설치 스크립트, 임의의 다운로드 사이트, 출처 불명의 파일, 비공식 패키지는 사용하지 마.
- 설치, 업데이트, 설정 실패 시 이미 성공한 도구나 기존 설정을 되돌리거나 덮어쓰지 마. 원인과 안전한 다음 행동을 설명한 뒤 필요한 경우에만 질문해.

다음 절차를 순서대로 진행해.

1. 먼저 `claude --version`과 `gh --version`을 각각 확인해. 명령을 찾지 못한 경우와 실행은 되지만 오류가 나는 경우를 구분해. 그리고 `winget --version`으로 winget을 쓸 수 있는지도 확인해.

2. Claude Code CLI를 확인하고 필요하면 설치해.
...
3. GitHub CLI를 확인하고 필요하면 설치해.
   ...
   - winget을 쓸 수 없으면 비공식 설치 파일을 받지 말고, 그 사실과 안전한 다음 조치를 알려 준 뒤 멈춰.
...
5. 로그인 두 가지를 차례로 처리해.
   ...
   5-1. GitHub CLI 로그인을 `gh auth status`로 확인해.
